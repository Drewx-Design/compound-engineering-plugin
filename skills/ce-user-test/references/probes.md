...
When invoked as `/ce-user-test N`, the skill orchestrates N sequential runs with inter-run probe learning.
...
If `/ce-user-test N` is interrupted at run K:
- The last-run JSON contains probe state through run K
- Run `/ce-user-test-commit` to persist what exists
- Probes generated during the interrupted run get status `untested`
- No special resume logic — next `/ce-user-test` reads probes from the test file
