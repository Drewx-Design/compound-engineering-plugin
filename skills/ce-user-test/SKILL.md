...
After displaying the report (and optional Proof sharing), write the rendered report text to `tests/user-flows/.user-test-last-report.md`. This file is the eval artifact — `/ce-user-test-eval` reads it to grade presentation-layer behavior. Overwritten each run, gitignored.
...
**Opt-out:** If invoked with `--no-commit` or if the run was partial (interrupted before all areas scored), skip commit and display the report only. The user can run `/ce-user-test-commit` later to commit from `.user-test-last-run.json`.
...
Runs automatically after Phase 4 completes a full run. Can also be invoked standalone via `/ce-user-test-commit` (e.g., after a `--no-commit` run or to retry a failed commit).
...
```
Run `/ce-user-test-eval` to grade this session's output against binary evals.
```
...
**Standalone `/ce-user-test-commit`:** Also displays the eval prompt after commit completes.
...
After final run, auto-commit (same as normal `/ce-user-test`). Pass `--no-commit` to skip.
