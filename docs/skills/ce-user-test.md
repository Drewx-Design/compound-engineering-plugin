...
- **Evidence-backed scores** — scored areas write typed evidence entries (`action`, `dom`, `timing`, `count`) into `.user-test-last-run.json` so UX and Quality scores are grounded in observed behavior
- **Anomaly ledger** — Phase 3 appends incidental anomalies or explicit `none` lines to `.user-test-anomalies.jsonl`; Phase 4 routes each anomaly as filed, noted in-area, explore-next-run, or dismissed with reason
...
- **Artifact-separated self-eval** — `/ce-user-test-eval` grades from file artifacts only, including ledger-to-report coverage, never conversation context, so grading integrity survives
